...
import com.dlf.common.utils.CommonUtils;
...
//        function.setLevel(FunctionEnums.function_level_2.getCode());
//        function.setType(FunctionEnums.function_type_1.getCode());
//        function.setParentCode(reqDTO.getParentCode());
//        List<FunDTO> userMenuList = functionMapper.queryListByParams(function);
//        if(CollectionUtils.isEmpty(userMenuList)){
//            for(FunDTO thisDTO : defaultMenuList){
//                Function menuInsert = new Function();
...
//            }
//            userMenuList = functionMapper.queryListByParams(function);
...
//                        //如果不存在，则更新数据为删除状态
//                    }else{
//                        Function funUpdate = new Function();
...
//                        Function funInsert = new Function();
...
//    }
    ...
    public GlobalResultDTO bindingMenuByUser(FunReqDTO reqDTO) {
        ...
        function.setLevel(FunctionEnums.function_level_2.getCode());
        function.setParentCode(reqDTO.getParentCode());
        List<FunDTO> orgMenuList = functionMapper.queryListByParams(function);
        //对比，更新状态
        for(FunDTO thisDTO : orgMenuList){
            ...
            if(checkedIdSet.contains(thisDTO.getId()) && thisDTO.getIsDeleted().intValue() == GlobalEnum.IS_DELETED_1.getCode().intValue()){
                funUpdate = new Function();
                BeanUtils.copyProperties(thisDTO, funUpdate);
                funUpdate.setIsDeleted(GlobalEnum.IS_DELETED_0.getCode());
                functionMapper.updateByPrimaryKey(funUpdate);
    ...
    public GlobalResultDTO queryMenuByParams(FunSearchDTO searchDTO) {
        ...
        function.setType(FunctionEnums.function_type_1.getCode());
//        function.setLevel(FunctionEnums.function_level_1.getCode());
        function.setIsDeleted(GlobalEnum.IS_DELETED_0.getCode());
    ...
    public GlobalResultDTO add(FunReqDTO reqDTO) {
        ...
        //parentCode + code
        fun.setCode(fun.getParentCode() + "-" + ((null == functionMapper.selectLastId() ? 0 : functionMapper.selectLastId()) + 1));
